...
I solve the neoclassial growth model with CRRA preferences and Cobb-Douglas production.  The state variable is $k$, the capital-labor ratio.  The value function for the problem is given by:

$$ V(k) = \max_{k'>0} \left\{ \frac{c^{1-\sigma}}{1-\sigma} + \beta V(k')  \right\} \\
  \text{s.t.} \qquad k' =  A k^\alpha - c + (1-\delta) k$$

This example is convenient because when $\sigma=1$ (so preferences over consumption are logarithmic) and $\delta=1$, then the model has a known following analytical solution.  This means that we can check the approximations relative to the exact truth in this case.
