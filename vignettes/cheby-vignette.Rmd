...
base <- d1.poly( fn=log, range=c(0,4), iOrder=6, iPts=10 )
```

This computes a 6th-order approximation to `log` on the interval $[0,4]$ using 10 grid points.   The output is a function that we can use straight away.  We can check the approximation at a few points:
...
```{r}
plot( log, type='l', lwd=2 ) 
plot( base, col='blue', lwd=2, add=TRUE )
```
